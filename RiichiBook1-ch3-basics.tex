...
\index{run ({\jap shuntsu}; chii)}
\index{chii ({\jap shuntsu}; run)}
\index{shuntsu@{\jap shuntsu} (run; chii)}
\index{set ({\jap kotsu}; pon)}
\index{pon ({\jap kotsu}; set)}
\index{kotsu@{\jap kotsu} (set; pon)}
\index{mentsu@{\jap mentsu} (group)}
\index{group ({\jap mentsu})}
\index{kan@{\jap kan} (kan)} \index{kantsu@{\jap kantsu} (quad)}
...
Groups can be classified into two kinds --- {\bf run} and {\bf set}.
\bi
\i Run ({\jap shuntsu}; chii / sequence) is a set of three consecutive number tiles: e.g., {\LARGE \wan{7}\wan{8}\wan{9}}, {\LARGE \suo{3}\suo{4}\suo{5}}. 
\i Set ({\jap kotsu}; pon / triplet) is a set of three identical tiles: e.g., {\LARGE \wan{2}\wan{2}\wan{2}}, {\LARGE \fa\fa\fa}.\footnote{Technically speaking, there is a third type of groups, namely quad ({\jap kantsu}; kan), a set of four identical tiles. We treat quads as a variant of sets. See Section \ref{sec:kan} for discussions on this.} 
\ei
...
\end{itembox}
\noindent
The hand is now ready. We should discard {\LARGE\suo{8}} and call riichi. If we win on {\LARGE\wan{9}}, we can claim riichi, {\jap pinfu}, and {\jap sanshoku} (Mixed Triple Sequence), giving us 7700 points.\footnote{We will discuss scoring and {\jap yaku} more extensively in later chapters.}
...
\item[Run (chii / sequence; {\jap shuntsu})] is a set of three consecutive number tiles.
\item[Set (pon / triplet; {\jap kotsu})] is a set of three identical tiles. 
\item[Quad (kan; {\jap kantsu})] is a set of four identical tiles. 
